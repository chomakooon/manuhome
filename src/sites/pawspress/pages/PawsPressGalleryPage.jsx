 ...
 *
 * /pet/gallery: もふらぼ の制作事例ページ。
 * pet 配下の portfolioItems を「お写真 / イラスト / グッズ」の3グループに
 * 分けてグリッド表示する。サムネイルをタップで PortfolioModal を開き、
 * グループ横断で前後送りできる。
 *
 * 「もっと事例を見る」のリンク先（旧 /portfolio → カタチらぼ側に遷移していた）
 * を もふらぼ 内で完結させるための独立ページ。
 */
...
export default function PawsPressGalleryPage() {
    ...
    const modalItem = isModalOpen
        ? {
              ...petItems[modalIndex],
              categoryName:
                  SUB_CATEGORY_LABEL[petItems[modalIndex].subCategory] ?? 'もふらぼ',
          }
        : null;
